...
$prix_class = 'prix ajenson';
$prix_parts = explode('$',$prix);
if(count($prix_parts)>1)
{
	$prix_class .= ' no_after';
	$prix = $prix_parts[0].'$ <small class="sub">'.$prix_parts[1].'</small>';
}
...
$html .= $content['media_wrapper_start']; // open the media markup
	...
	$html .= '<a class="tg-item-big-link" href="'.get_permalink($gridItem['ID']).'">';
	...
	//$html .= '<pre>Meta: '.print_r($gridItem['ID'], true).'</pre>';
	...
	$html .= '<div class="tg-item-content">'; // we place the content holder over the media and overlay
		...
		$html .= '<div class="tg-item-footer">'; // a custom markup to make a footer
			...
			$html .= '<div class="'.$prix_class.'">'.$prix.'</div>';
